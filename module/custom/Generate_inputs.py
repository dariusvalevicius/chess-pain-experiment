...
def create_subset(elo_player,participant_id):
    ...
    easy_puzzles = easy_puzzles.reindex(columns =easy_puzzles.columns.tolist() + ["correct_answer", "timed_out", "time_elapsed"])  
    ...
    subset_flow_clean = subset_flow_clean.reindex(columns =subset_flow_clean.columns.tolist() + ["correct_answer", "timed_out", "time_elapsed"])  
    ...
    subset_hard_clean = subset_hard_clean.reindex(columns =subset_hard_clean.columns.tolist() + ["correct_answer", "timed_out", "time_elapsed"])  
    ...
    easy_puzzles.to_csv('easy_set.tsv', sep='\t', index=False)
    subset_flow_clean.to_csv('flow_set.tsv', sep='\t', index=False)
    subset_hard_clean.to_csv('hard_set.tsv', sep='\t', index=False)
