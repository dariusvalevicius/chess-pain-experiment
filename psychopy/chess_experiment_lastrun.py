...
chess_feedback = visual.TextStim(win=win, name='chess_feedback',
    text=feedback_text,
    font='Open Sans',
    pos=(-6/8, 0), height=0.05, wrapWidth=None, ori=0.0, 
...
puzzle_timer = visual.TextStim(win=win, name='puzzle_timer',
    text=puzzle_time,
    font='Open Sans',
    pos=(-6/8, 3/8), height=0.05, wrapWidth=None, ori=0.0, 
    color='white', colorSpace='rgb', opacity=None, 
    languageStyle='LTR',
    depth=-4.0);
...
for thisChess_trial in chess_trials:
    ...
    # keep track of which components have finished
    chess_puzzleComponents = [mouse, chess_board, chess_feedback, puzzle_timer]
    ...
    while continueRoutine:
        ...
        if timeout:
            ...
            puzzle_timer.text = str(round(puzzle_time))
        ...
        if puzzle_timer.status == NOT_STARTED and tThisFlip >= 0.0-frameTolerance:
            ...
            # add timestamp to datafile
            thisExp.timestampOnFlip(win, 'puzzle_timer.started')
